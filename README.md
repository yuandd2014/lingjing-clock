...
> **灵境时钟** 是一款高定制的 Windows 桌面待屏时钟，搭载自研 **灵境UI** 设计语言与 **灵镜光感** 视觉效果。
> 集成超大数字时钟、实时天气、农历日历、节气/每日一言、Live2D 虚拟形象和粒子背景。
...
**灵境UI** 是本项目自研的桌面 UI 设计语言，核心理念是"屏幕如镜，光从内生"。

### 灵镜光感（视觉效果）
- **毛玻璃** — `backdrop-filter: blur(40px) saturate(1.8)` 多层叠加
- **粒子背景** — 白色细粒从底部向上漂浮，模拟空气中的尘埃
- **模糊发光** — 文字采用多层 `text-shadow` 与 `drop-shadow`，呈现"被光渗透"的质感
- **暗角** — 中心透明、边缘渐黑的径向遮罩，强化"窗中窥镜"的纵深

### 布局语言
- 3×2 网格，玻璃卡片化分区
- 钟体占据左侧 1.2 列、跨 2 行，给数字以"主角感"
- 天气 / 日历 / 每日一言 三块灵镜光感卡片，环形呼应

## 🎯 功能特性

- **🕐 超大数字时钟** — Orbitron 字体 + 多层光晕，灵镜光感
- **☁️ 三源容错天气** — 和风天气（国内主力）+ wttr.in + open-meteo 多级降级
- **📅 高精度农历/节气** — 基于 GitHub 高精度算法，公历/农历双向转换、干支、生肖
- **💬 每日一言** — 1265 句中外名言，30 天内不重复，点击卡片可手动刷新
- **🌸 Live2D 虚拟形象** — Cubism 4 SDK，支持 Hiyori / shizuku 模型
- **✨ 灵镜光感背景** — 动态漂浮粒子 + 毛玻璃，独家视觉签名
...
灵境UI 是本项目**原创**的桌面视觉设计语言，灵镜光感是它的核心视觉效果。

与"灵动岛"、"光遇"等命名套路相同——这是项目对自身视觉风格的命名，不是对任何已有操作系统或产品的仿写。
